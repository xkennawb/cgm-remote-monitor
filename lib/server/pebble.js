...
function addExtraData (first, req, sbx) {
  ...
  function addSensorAge() {
    if (req.sage) {
      var sageInfo = sbx.properties.sage;
      if (sageInfo && sageInfo.min) {
        var sensorData = sageInfo[sageInfo.min];
        if (sensorData && sensorData.found) {
          first.sensorAge = sensorData.display || '';
          first.sensorDays = sensorData.days || 0;
          first.sensorHours = sensorData.hours || 0;
          // Calculate expiry for Dexcom G7 (10.5 days)
          var totalHours = sensorData.age || 0;
          var expiryHours = 252; // 10.5 days for G7
          var remainingHours = Math.max(0, expiryHours - totalHours);
          first.sensorRemaining = Math.floor(remainingHours / 24) + 'd' + (remainingHours % 24) + 'h';
        }
      }
    }
  }
...
function prepareSandbox (req) {
  ...
  // Add sensor age properties if enabled
  if (req.sage && req.ctx.plugins('sage')) {
    req.ctx.plugins('sage').setProperties(sbx);
  }
...
function configure (env, ctx) {
  ...
  function middle (req, res, next) {
    ...
    req.sage = env.settings.isEnabled('sage');
